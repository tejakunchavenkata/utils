...
function GetVerilog_SystemVerilogIndent()
  ...
  if last_line =~ '\*/\s*$' && last_line !~ '/\*.\{-}\*/'
    ...
    " Indent after if/else/for/case/always/initial/specify/fork/while blocks
  elseif last_line =~ '`\@<!\<\(if\|else\)\>' ||
        \ last_line =~ '^\s*\<\(for\|case\%[[zx]]\|do\|foreach\|while\|randsequence\|randcase\)\>' ||
  ...
  elseif last_line =~ '^\s*\(\<\(virtual\|protected\|static\)\>\s\+\)*\s*\<\(function\|task\|class\|package\)\>' ||
  ...
  elseif last_line =~ '\(\<begin\>\)\(\s*:\s*\w\+\)*' . vlog_comment . '*$' &&
    ...
    " Indent after a '{' or a '('
    " Last line has opening Brace
    " Opening brace is not in comment
    " Why these ???
    " \ ( last_line2 !~ vlog_openstat . '\s*' . vlog_comment . '*$' ||
    " \ last_line2 =~ '^\s*[^=!]\+\s*:\s*' . vlog_comment . '*$' )
  elseif last_line =~ '[{(]' . vlog_comment . '*$' &&
        \ last_line !~ '\(//\|/\*\).*[{(]'
    let ind = ind + offset
    if vverb | echo vverb_str "Indent after '{' or '('" | endif
  ...
  elseif ( last_line !~ '\<begin\>' ||
    ...
    elseif curr_line =~ '^\s*\<begin\>'
      if last_line !~ '^\s*\<\(function\|task\|specify\|module\|class\|package\)\>' ||
            ...
            \ last_line !~ '^\s*\()*\s*;\|)\+\)\s*' . vlog_comment . '*$' &&
            \ ( last_line =~
            \ '\<\(`\@<!if\|`\@<!else\|for\|case\%[[zx]]\|always\|initial\|do\|foreach\|randsequence\|randcase\|final\)\>' ||
              \ last_line =~ ')\s*' . vlog_comment . '*$' ||
